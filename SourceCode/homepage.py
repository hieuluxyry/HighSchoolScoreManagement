...
class HomePage(Tk):
    def __init__(self, db_connection):
        ...
        label_title = Label(self, text="Chào mừng bạn đến với Quản lý điểm hệ THPT !!! ", font=("Time New Roman", 20))
        label_title.config(bg="white",fg="red")
        label_title.pack(pady=10)

        button_add_class = Button(self, text="Thêm lớp học", command=self.open_add_class_form)
        button_add_class.config(bg="white",fg="red")
        button_add_class.place(x=20, y=40, width=100, height=25)

        button_add_student = Button(self, text="Thêm học sinh", command=self.open_add_student_form)
        button_add_student.config(bg="white",fg="red")
        button_add_student.place(x=20, y=80, width=100, height=25)

        button_add_subject = Button(self, text="Thêm môn học", command=self.open_add_subject_form)
        button_add_subject.config(bg="white",fg="red")
        ...
        button_add_score.config(bg="white",fg="red")
        button_add_score.place(x=20, y=170, width=100, height=25)

        button_view_scores = Button(self, text="Xem điểm", command=self.open_view_scores_form)
        button_view_scores.config(bg="white",fg="red")
        button_view_scores.place(x=20, y=230, width=100, height=25)

        button_logout = Button(self, text="Đăng xuất", command=self.logout)
        button_logout.config(bg="white",fg="red")
        button_logout.place(x=20, y=280, width=100, height=25)
